/*
 * $Id: cmpiSMIS_FCElementStatisticalDataProvider.c,v 1.1.1.1 2009/05/12 21:46:34 nsharoff Exp $
 *
 ...
 *
 * Description:
 *      Provider for the Fibre Channel Port for which statistical or metric
 *	data is defined.
*/
...
static char * _ClassName     = "Linux_FCElementStatisticalData";
static char * _RefLeft       = "ManagedElement";
static char * _RefRight      = "Stats";
static char * _RefLeftClass  = "Linux_FCPort";
static char * _RefRightClass = "Linux_FCPortStatistics";



/* ---------------------------------------------------------------------------*/
/*                      _assoc_create_refs_FCElementStatisticalData()         */
/* ---------------------------------------------------------------------------*/
...
int _assoc_create_refs_FCElementStatisticalData( const CMPIBroker * _broker,
   ...
   int                  getStatistics;     /* whether to get statistics data from hba */

   _OSBASE_TRACE(2,("--- _assoc_create_refs_FCElementStatisticalData() called"));
   ...
   if( (associators == 1) && (inst == 1) && (targetName==_RefRightClass) )
      getStatistics=TRUE;
   else
      getStatistics=FALSE;

   if( enum_all_hbaPorts( &lptr, getStatistics ) != 0 ) 
      {
      CMSetStatusWithChars( _broker, rc,
                            CMPI_RC_ERR_FAILED, "Could not list hba ports." );
      _OSBASE_TRACE(2,("--- _assoc_create_refs_FCElementStatisticalData() exited"));
      return -1;
      }


   /* check if source instance does exist */
   /* For this association it is a one to one mapping between FCPort and FCPortStatistics */
   ...
   for ( ; lptr && rc->rc == CMPI_RC_OK ; lptr = lptr->next) 
      ...
      if (sourceName==_RefLeftClass)
         ...
         sop = _makePath_FCPortStatistics( _broker, ctx, ref,lptr->sptr, rc );
   ...
   if( (associators == 1) && (inst == 0) ) 
      ...
      if (targetName==_RefLeftClass)
         ...
         top = _makePath_FCPortStatistics( _broker, ctx, ref,lptr->sptr, rc );
   ...
   if( (associators == 1) && (inst == 1) ) 
      ...
      if (targetName==_RefLeftClass)
         ...
         tin = _makeInst_FCPortStatistics( _broker, ctx, ref,lptr->sptr, rc );
   ...
   if( (associators == 0) && (inst == 0) ) 
      ...
      rop = _makePath_FCPortStatistics( _broker, ctx, ref,lptr->sptr, rc );
      if( rc->rc != CMPI_RC_OK ) rop=NULL;		 
	    
      if( cop == NULL || lop == NULL || rop == NULL) 
	 {
         _OSBASE_TRACE(1,("--- %s CMPI _assoc_create_refs_FCElementStatisticalData() failed creating object paths.",_ClassName));
         CMSetStatusWithChars( _broker, rc,
                               CMPI_RC_ERR_FAILED, "Create CMPIObjectPath failed." );
         if(rm) free_hbaPortList(rm);
         _OSBASE_TRACE(1,("--- %s CMPI _assoc_create_refs_FCElementStatisticalData() failed : %s",_ClassName,CMGetCharPtr(rc->msg)));
         return -1;
         }
   ...
   if( (associators == 0) && (inst == 1) ) 
	 ...
	 {
         _OSBASE_TRACE(1,("--- %s CMPI _assoc_create_refs_FCElementStatisticalData() failed creating object paths or instances.",_ClassName));
         CMSetStatusWithChars( _broker, rc,
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderCleanup                     */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderCleanup( CMPIInstanceMI * mi,
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderEnumInstanceNames           */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderEnumInstanceNames( CMPIInstanceMI * mi,
            const CMPIContext * ctx,
            const CMPIResult * rslt,
            const CMPIObjectPath * ref) 
   {
   ...
   return rc;
   } /* end SMIS_FCElementStatisticalDataProviderEnumInstanceNames */
   
   
   

/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderEnumInstances               */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderEnumInstances( CMPIInstanceMI * mi,
            const CMPIContext * ctx,
            const CMPIResult * rslt,
            const CMPIObjectPath * ref,
   ...
   return rc;
   } /* end SMIS_FCElementStatisticalDataProviderEnumInstances */
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderGetInstance                 */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderGetInstance( CMPIInstanceMI * mi,
            const CMPIContext * ctx,
            const CMPIResult * rslt,
   ...
   return rc;
   } /* end SMIS_FCElementStatisticalDataProviderGetInstance */
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderCreateInstance              */
...
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderCreateInstance( CMPIInstanceMI * mi,
            const CMPIContext * ctx,
            const CMPIResult * rslt,
            const CMPIObjectPath * cop,
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderModifyInstance              */
/* Notes: This method is only here as a stub in case we later support some    */
/*        methods and not others, and to catch a call to this method in case  */
/*        it is not set to unsupported (or no methods supported) elsewhere.   */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderModifyInstance( CMPIInstanceMI * mi,
            const CMPIContext * ctx,
            const CMPIResult * rslt,
            const CMPIObjectPath * cop,
            const CMPIInstance * ci,
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderDeleteInstance              */
/* Notes: This method is only here as a stub in case we later support some    */
/*        methods and not others, and to catch a call to this method in case  */
/*        it is not set to unsupported (or no methods supported) elsewhere.   */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderDeleteInstance( CMPIInstanceMI * mi,
            const CMPIContext * ctx,
            const CMPIResult * rslt,
            const CMPIObjectPath * cop) 
   {
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderExecQuery                   */
/* Notes: This method is only here as a stub in case we later support some    */
/*        methods and not others, and to catch a call to this method in case  */
/*        it is not set to unsupported (or no methods supported) elsewhere.   */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderExecQuery( CMPIInstanceMI * mi,
            const CMPIContext * ctx,
            const CMPIResult * rslt,
            const CMPIObjectPath * ref,
            const char * lang,
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderAssociationCleanup          */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderAssociationCleanup( 
 ...
 *    < role >   -> < class >
 *    ManagedElement -> Linux_FCPort
 *    Stats          -> Linux_FCPortStatistics
 *
 */
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderAssociators                 */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderAssociators( CMPIAssociationMI * mi,
           const CMPIContext * ctx,
           const CMPIResult * rslt,
           const CMPIObjectPath * cop,
   ...
   /* There are no children classes of FCPort or FCPortStatistics, so the only resultClass that */
   /*    makes sense is the _RefRightClass or _RefLeftClass */

   refrc = _assoc_create_refs_FCElementStatisticalData(_broker, ctx, rslt, cop, 
   ...
   CMReturn(CMPI_RC_OK);
   } /* end SMIS_FCElementStatisticalDataProviderAssociators */
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderAssociatorNames             */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderAssociatorNames( CMPIAssociationMI * mi,
           const CMPIContext * ctx,
           const CMPIResult * rslt,
           const CMPIObjectPath * cop,
           const char * assocClass,
   ...
   CMReturn(CMPI_RC_OK);
   } /* end SMIS_FCElementStatisticalDataProviderAssociatorNames */
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderReferences                  */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderReferences( CMPIAssociationMI * mi,
           const CMPIContext * ctx,
           const CMPIResult * rslt,
           const CMPIObjectPath * cop,
           const char * assocClass,
   ...
   CMReturn(CMPI_RC_OK);
   } /* end SMIS_FCElementStatisticalDataProviderReferences */
...
/******************************************************************************/
/* Function: SMIS_FCElementStatisticalDataProviderReferenceNames              */
/******************************************************************************/
CMPIStatus SMIS_FCElementStatisticalDataProviderReferenceNames( CMPIAssociationMI * mi,
           const CMPIContext * ctx,
           const CMPIResult * rslt,
           const CMPIObjectPath * cop,
           const char * assocClass,
   ...
   CMReturn(CMPI_RC_OK);
   } /* end SMIS_FCElementStatisticalDataProviderReferenceNames */
...
CMInstanceMIStub( SMIS_FCElementStatisticalDataProvider,
                  SMIS_FCElementStatisticalDataProvider,
                  _broker,
                  CMNoHook)

CMAssociationMIStub( SMIS_FCElementStatisticalDataProvider,
                SMIS_FCElementStatisticalDataProvider,
                _broker,
                CMNoHook)
...
/* ---------------------------------------------------------------------------*/
/*                end of cmpiSMIS_FCElementStatisticalDataProvider                              */
/* ---------------------------------------------------------------------------*/
